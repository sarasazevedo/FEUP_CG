...
export class MyScene extends CGFscene {
    ...
    init(application) {
        ...
        this.objects = [this.tangram, this.plane, this.pyramid, this.cone, this.unitCube, this.prism, this.cylinder];

        this.objectIDs = { 'Tangram': 0, 'Plane': 1, 'Pyramid': 2, 'Cone': 3, 'Unit Cube': 4 , 'Prism': 5, 'Cylinder': 6};
    ...
    updateObjectComplexity(){
        if (this.objects[this.selectedObject] && 
            typeof this.objects[this.selectedObject].updateBuffers === 'function') {
    ...
    initMaterials() {
        ...
        this.material4.setAmbient(0.3, 0.2, 0.1, 1);
        this.material4.setDiffuse(0.6, 0.4, 0.2, 1);
        this.material4.setSpecular(0.05, 0.05, 0.05, 1);
...
display() {
    ...
    // Check if the selected object is the Tangram and position it properly
    if (this.selectedObject === 0) { // 4 is the index of Tangram
        // Reset to a position where it's clearly visible
        this.translate(0, 0, 0);
        this.scale(1, 1, 1); // Use smaller scale for Tangram if needed
    } else {
    ...
    try {
        ...
        console.error("Error displaying object:", error);
    }
