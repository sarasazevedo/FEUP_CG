...
export class MyPlane extends CGFobject {
	...
	initMaterials() {
		...
		const planeTexture = this.scene.textureHandler?.getTexture('plane');
		
		if (planeTexture) {
			this.material.setTexture(planeTexture);
		} 
	...
	initBuffers() {
		...
		for (var j = 0; j <= this.nrDivs; j++) {
			...
			for (var i = 0; i <= this.nrDivs; i++) {
				...
				// Use repeat for texture coordinates - multiply by a constant to repeat texture
				this.texCoords.push((this.minS + i * this.q) * texRepeat, (this.minT + j * this.w) * texRepeat);
