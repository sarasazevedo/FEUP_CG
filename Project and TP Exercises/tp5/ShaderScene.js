...
export class ShaderScene extends CGFscene {
	constructor() {
		...
		this.selectedObject = 1;  // Changed from 0 to 1 to select Plane by default
		this.wireframe = false;
		this.selectedExampleShader = 11;  // Changed from 0 to 11 to select Water shader by default
	...
	init(application) {
		...
		this.testShaders = [
			...
			new CGFshader(this.gl, "shaders/myvertex.vert", "shaders/myfrang.frag"),
			new CGFshader(this.gl, "shaders/myvertex.vert", "shaders/myfrang2.frag"),
		...
		// Initialize water shader uniforms
	...
	update(t) {
		...
		// Update timeFactor for water shader
	...
	display() {
		...
		this.pushMatrix();
		 ...
		 // Apply the correct texture based on the selected shader
		if (this.selectedExampleShader == 11) {
			// Bind water textures for the water shader
			this.waterTex.bind(0);  
			this.waterMap.bind(1);
		} else {
			// For other shaders, use the regular textures
			this.texture.bind(0);
